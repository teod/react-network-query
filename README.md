...
| headers?                |    -    | `{ [key: string]: string }` | Default headers to be attached to each network call make by `<Query />` and `<Mutaion />` components.                                              |
| persistentStorage?      |    -    |     `PersistentStorage`     | A storage interface, when passed data will be automatically persisted into the spcified storage, it works only with `window.localStorage` for now. |
...
| headers?    |    -    |       `{ [key: string]: string }`        | Additional headers to be attached to the network request.                                                                                                                                                                                                              |
...
| Prop        | Default |                                          Type                                          | Description                                                                                                                                                                                                                                                 |
| :---------- | :-----: | :------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| children    |    -    | `({ update, isMutating }: { update: UpdateArg, isMutation: boolean }) => ReactElement` | Required prop, needs to be a function that will return valid react elements, it provides an `update` function to do the network request and an `isMutating` state for showing the current status of the network request.                                    |
